...
[Version = "1.0.0"]
section AmazonOpenSearchService;
...
[DataSource.Kind="AmazonOpenSearchService", Publish="AmazonOpenSearchService.Publish"]
shared AmazonOpenSearchService.Contents = Value.ReplaceType(AmazonOpenSearchServiceImpl, AmazonOpenSearchServiceType);

// Wrapper function to provide additional UI customization.
AmazonOpenSearchServiceType = type function (
...
AmazonOpenSearchServiceImpl = (Server as text, Port as number, UseSSL as logical) as table =>
    let
...
AmazonOpenSearchService = [
    ...
    TestConnection = (dataSourcePath) => 
        let
            ...
            UseSSL = json[UseSSL]
        in
            { "AmazonOpenSearchService.Contents", Server, Port, UseSSL },
    ...
    DSRHandlers = [
      amazonopensearchservice = [
        GetDSR = (server, schema, object, optional options) => [ protocol = "amazonopensearchservice-odbc", address = [ server = server, port = schema, useSSL = object ] ],
        GetFormula = (dsr, optional options) => () =>
          let
            db = AmazonOpenSearchService.Contents(dsr[address][server], dsr[address][port], dsr[address][useSSL])
          in
            db,
        GetFriendlyName = (dsr) => "Amazon OpenSearch Service"
      ]
    ],
...
AmazonOpenSearchService.Publish = [
    Beta = true,
    Category = "Other",
    ButtonText = { Extension.LoadString("ButtonTitle"), Extension.LoadString("ButtonHelp") },
    LearnMoreUrl = "https://github.com/opensearch-project/sql/blob/main/bi-connectors/PowerBIConnector/docs/AmazonOpenSearchService.md",
